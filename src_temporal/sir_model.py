...
def run_sir_analysis():
    ...
    if grp_col is None:
        raise KeyError(f"❌ Cannot find session ID column! Available columns: {df.columns.tolist()}")
    
    print(f"✅ Using '{grp_col}' as session identifier.")
    # ------------------------------------

    print("Preparing SIR data (Aligning sessions)...")
    ...
    if valid_sessions == 0:
        print("⚠️ No 10-person sessions found! Cannot fit standard SIR.")
        return
    ...
    try:
        ...
        print(f"📈 SIR plot saved to: {save_path}")
